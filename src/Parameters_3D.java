...
public void run(ImageProcessor ip) {
		...
		SaveDialog sd = new SaveDialog("Save Measurements in file as Text...", "res", ".dat");
...
//Find the highest value in the volume and return it
public int calculnb(final ImageStack stack1)
{
	...
	final int[] value  = new int[threads.length];
	int tmp=(int)(dimZ/threads.length);
	if(dimZ%threads.length!=0)
		tmp =(int)(dimZ/threads.length)+1;
	final int pitch = tmp;
	for (int ithread = 0; ithread < threads.length; ithread++) {
		...
		{	
			for (int superi = ai.getAndIncrement(); superi < threads.length; superi = ai.getAndIncrement()) {
				...
				{
					if(current*pitch+k<=dimZ)
					{
						ImageProcessor ip = stack1.getProcessor(current*pitch+k);
						for(int  j=0;j<dimY;j++)
						{
							for(int  i=0;i<dimX;i++)
							{
								int val =(int) (ip.getPixelValue(i,j));
								if (val>value[current]) value[current]=val;
							}
						}
					}
				}
...
//Compute marching cubes for surface and volume
void calculsurfacemarch2stack(final ImageStack stack1, int nb, final double[] tri, double[] s, final double[] tri2, double[] v)
{
	...
	final int dimY = stack1.getHeight();
	final int dimZ = stack1.getSize();
	final double[][] vol  = new double[threads.length][nb];
	final double[][] surf  = new double[threads.length][nb];
	int tmp=(int)((dimZ)/threads.length); //dimZ+2
	if((dimZ)%threads.length!=0)//dimZ+2
		tmp =(int)((dimZ)/threads.length)+1;//dimZ+2
	final int pitch = tmp;
	final int[] test = new int[dimZ];//dimZ+2
	for (int ithread = 0; ithread < threads.length; ithread++) {
		...
		{	
			for (int superi = ai.getAndIncrement(); superi < threads.length; superi = ai.getAndIncrement()) {
				...
				for(int ll=1;ll<=pitch;ll++)
				{
					int k=current*pitch+ll-2;
					if(k<=dimZ)
					{
						test[k+1]=k;
					...
					for(int j=-1;j<dimY;j++)
					{
						for(int i=-1;i<dimX;i++)
						{
					        ...
					        for(int l=0;l<8;l++)
					        {
					        	if (p[l]!=0 && appart(p[l],nontab,cpt)==1)
					        	{
					        		nontab[cpt]=p[l];
					        		cpt++;
					        	}
					        ...
					        for(int mm=0;mm<cpt;mm++)
					        {
					              ...
					              if (pixcoul!=0)
					              {
					            	  ...
					            	  surf[current][(int)(pixcoul-1)]+=tri[(int)(ptot)];	
					            	  vol[current][(int)(pixcoul-1)]+=tri2[(int)(ptot)];
					              } 
					        } 
						}
					}
	...
	IJ.showMessage(" "+compt+" errors");
	*/
}

//Old parallelized one
void calculsurfacemarch2stackp(final ImageStack stack1, int nb, final double[] tri, double[] s, final double[] tri2, double[] v)
{
...
void calculsurfacemarch3stack(final ImageStack stack1, int nb, final double[] tri,double[] s2)
{
	...
	final int dimZ = stack1.getSize();
	final double[][] surf  = new double[threads.length][nb];
	int tmp=(int)((dimZ-1)/threads.length);
	if((dimZ-1)%threads.length!=0)
		tmp =(int)((dimZ-1)/threads.length)+1;
	final int pitch = tmp;
	...
	for (int ithread = 0; ithread < threads.length; ithread++) {
		...
		{	
			for (int superi = ai.getAndIncrement(); superi < threads.length; superi = ai.getAndIncrement()) {
				...
				for(int ll=1; ll<=pitch; ll++)
				{
					int k= current*pitch+ll;	
					if(k< dimZ){
				ImageProcessor ip1,ip2;
				ip1 = stack1.getProcessor(k);
				ip2 = stack1.getProcessor(k+1);
...
//Compute volume of each gray level higher than 0 and return it in v (with volume of object labeled i in v[i-1]
void calculvolumestack(final ImageStack stack1, int nb, int v[])
{
	...
	final int dimY = stack1.getHeight();
	final int dimZ = stack1.getSize();
	final int[][] vol  = new int[threads.length][nb];
	int tmp=(int)(dimZ/threads.length);
	if(dimZ%threads.length!=0)
		tmp =(int)(dimZ/threads.length)+1;
	final int pitch = tmp;
...
//Compute center of mass and volume of each gray level higher than 0 and return it in v (with volume of object labeled i in v[i-1]
void calculcgstack(final ImageStack stack1, int nb, final int[] v, final double[] xg, final double[] yg, final double[] zg)
{
	...
	final int dimZ = stack1.getSize();
	final int[][] vol  = new int[threads.length][nb];
	final int[][] tmpxg  = new int[threads.length][nb];
	final int[][] tmpyg  = new int[threads.length][nb];
	final int[][] tmpzg  = new int[threads.length][nb];
	int tmp=(int)(dimZ/threads.length);
	if(dimZ%threads.length!=0)
		tmp =(int)(dimZ/threads.length)+1;
	final int pitch = tmp;
		...
		{	
			for (int superi = ai.getAndIncrement(); superi <threads.length; superi = ai.getAndIncrement()) {
				...
				for(int ll=1;ll<=pitch;ll++)
				{
					...
					if(k<=dimZ)
					{
						ImageProcessor ip = stack1.getProcessor(k);
						for(int j=0;j<dimY;j++)
						{
							for(int i=0;i<dimX;i++)
							{
								...
								if(pix!=0){
						          tmpxg[current][pix-1] += i;
						          tmpyg[current][pix-1] += j;
						          tmpzg[current][pix-1] += k-1;
						          vol[current][pix-1]++;
						    }
						}
					}
...
void calculinertiestack(final ImageStack stack1, int nb, final double[] xg, final double[] yg, final double[] zg, double[][][] I)
{
	...
	final int dimY = stack1.getHeight();
	final int dimZ = stack1.getSize();
	final double[][][][] inert  = new double[threads.length][nb][3][3];
	int tmp=(int)(dimZ/threads.length);
	if(dimZ%threads.length!=0)
		tmp =(int)(dimZ/threads.length)+1;
	final int pitch = tmp;
					...
					if(k<=dimZ)
					{
						...
						for(int j=0;j<dimY;j++)
						{
							for(int i=0;i<dimX;i++)
							{
								int pix =(int) (ip.getPixelValue(i,j));
								if(pix!=0){
						         inert[current][pix-1][0][0]+=(j-yg[pix-1])*(j-yg[pix-1])+(k-1-zg[pix-1])*(k-1-zg[pix-1])+1.0/6.0;
						         inert[current][pix-1][0][1]-=(i-xg[pix-1])*(j-yg[pix-1]);
						         inert[current][pix-1][0][2]-=(i-xg[pix-1])*(k-1-zg[pix-1]);
						         inert[current][pix-1][1][1]+=(i-xg[pix-1])*(i-xg[pix-1])+(k-1-zg[pix-1])*(k-1-zg[pix-1])+1.0/6.0;
						         inert[current][pix-1][1][2]-=(j-yg[pix-1])*(k-1-zg[pix-1]);
						         inert[current][pix-1][2][2]+=(i-xg[pix-1])*(i-xg[pix-1])+(j-yg[pix-1])*(j-yg[pix-1])+1.0/6.0;
						    }
						}
					}
					}
...
void boitestack(final ImageStack stack1, final int nb, final double[] xg, final double[] yg, final double[] zg, final double[][][] dir, double[][] lmin, double[][] lmax)
{
	...
	final int dimY = stack1.getHeight();
	final int dimZ = stack1.getSize();
	final double[][][] lmint = new double[threads.length][nb][3];
	final double[][][] lmaxt = new double[threads.length][nb][3];
	...
	for (int ithread = 0; ithread < threads.length; ithread++) {
		...
		{	
			for (int superi = ai.getAndIncrement(); superi <threads.length; superi = ai.getAndIncrement()) {
				...
				for(int l=0; l<nb; l++)
				{
				  lmint[current][l][0]= 100000;
				  lmint[current][l][1]= 100000;
				  lmint[current][l][2]= 100000;
				  lmaxt[current][l][0]= -100000;
				  lmaxt[current][l][1]= -100000;
				  lmaxt[current][l][2]= -100000;
				}
				for(int ll=1;ll<=pitch;ll++)
				{
					int k=current*pitch+ll;
				...
				for(int j=0;j<dimY;j++)
					{
						for(int i=0;i<dimX;i++)
						{
							int pix =(int) (ip.getPixelValue(i,j));
							if(pix!=0){
							    double v1 = (i-xg[pix-1])*dir[0][0][pix-1]+(j-yg[pix-1])*dir[1][0][pix-1]+(k-1-zg[pix-1])*dir[2][0][pix-1];
							    double v2 = (i-xg[pix-1])*dir[0][1][pix-1]+(j-yg[pix-1])*dir[1][1][pix-1]+(k-1-zg[pix-1])*dir[2][1][pix-1];
							    double v3 = (i-xg[pix-1])*dir[0][2][pix-1]+(j-yg[pix-1])*dir[1][2][pix-1]+(k-1-zg[pix-1])*dir[2][2][pix-1];
							    if (v1<lmint[current][pix-1][0]) lmint[current][pix-1][0]=v1;
							    if (v1>lmaxt[current][pix-1][0]) lmaxt[current][pix-1][0]=v1;
							    if (v2<lmint[current][pix-1][1]) lmint[current][pix-1][1]=v2;
							    if (v2>lmaxt[current][pix-1][1]) lmaxt[current][pix-1][1]=v2;
							    if (v3<lmint[current][pix-1][2]) lmint[current][pix-1][2]=v3;
							    if (v3>lmaxt[current][pix-1][2]) lmaxt[current][pix-1][2]=v3;
						    }
						}
					}
					}
...
void borderstack(final ImageStack stack1, int nb, byte b[])
{
	...
	final int dimY = stack1.getHeight();
	final int dimZ = stack1.getSize();
	final byte[][] bord  = new byte[threads.length][nb];
				...
				for(int ll=1;ll<=pitch;ll++)
				{
					...
					if(k<=dimZ)
					{
						...
						for(int  j=0;j<dimY;j++)
						{
							...
							{
								int val =(int) (ip.getPixelValue(i,j));
								if ((val != 0) && ( i==0 || j==0 || k==1 || i==dimX-1 || j==dimY-1 || k==dimZ))
									//bord[k][val-2]=1;
									bord[current][val-1]=1;
							}
						}
					}
				}
