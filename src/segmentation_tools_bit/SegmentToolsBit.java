...
public class SegmentToolsBit {
	...
	ImageStack stacke = duplicate(stack);
	...
	if(element==el[1])
	{
		if (nbr>0)
		{
			for(int k = 0; k<nbr; ++k)
			{
				IJ.showStatus("Distance "+ (k+1));
				int[] modif = {0};
				stacke = StepMapOct(stacke, modif);
			}
		}
		else
		{
			...
			int[] modif = {1};
			int k=0;
			while(modif[0]!=oldmodif)
			{
				oldmodif=modif[0];
				modif[0]=0;
				k++;
				IJ.showStatus("Distance "+ k);
				stacke = StepMapOct(stacke, modif);
				//IJ.log("step " + k + " modif " + modif[0]);
			}
		}
	}
	else
	{
		if (nbr>0)
		{
			for(int k = 0; k<nbr; ++k)
			{
				int[] modif = {0};
				IJ.showStatus("Distance "+ (k+1));
				stacke = StepMapCube(stacke, modif);
			}
		}
		else
		{
  ...
  { 
    ImageStack dest = duplicate(labelized);
	ImageStack inter = duplicate(labelized);
	int MIN_VALUE = 0;
	int xMin = 0;
    int xMax = dimX;
    int yMin = 0;
  ...
  return dest;
	...
	private ImageStack StepMapOct(final ImageStack stack1, int[] follow){	
		
		final ImageStack stack2 = duplicateEmpty(stack1);
		final AtomicInteger ai = new AtomicInteger(1);
		final int[] change = new int[dimZ];
		for (int ithread = 0; ithread < threads.length; ithread++) {
			...
			for (int superi = ai.getAndIncrement(); superi <= dimZ; superi = ai.getAndIncrement()) {
				...
				int pix1, pix2, pix3, m;
					...
					for (int i=0; i<dimX*dimY; ++i)
					{
						...
						if (((int)pixelsSlice[i] & 0xff) != ((int)pixels[i] & 0xff))
						{
							pixelsSlice[i] = (byte)(((int)pixelsSlice[i] & 0xff) + 1);
							change[current-1] = change[current-1]+1;
						}
					}
				pixels = null;
				pixelsPlus = null ;
	...
	private ImageStack StepMapCube(final ImageStack stack1, int[] follow){
		final ImageStack stack2 = duplicateEmpty(stack1);
		final ImageStack stack3 = duplicateEmpty(stack1);
		final int[] change = new int[dimZ];
		final AtomicInteger ai = new AtomicInteger(1);
		for (int ithread = 0; ithread < threads.length; ithread++) {
			...
			for (int superi = ai2.getAndIncrement(); superi <= dimZ; superi = ai2.getAndIncrement()) {
				...
				byte[] pixelsRef;
					...
					for (int i=0; i<dimX*dimY; ++i)
					{
						...
						if (((int)pixelsRef[i] & 0xff) != ((int)pixels[i] & 0xff))
						{
							pixels[i] = (byte)(((int)pixels[i] & 0xff) + 1);
							change[current-1]=change[current-1]+1;
						}
					}
