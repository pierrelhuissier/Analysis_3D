				...
				for(int  j=0;j<dimY;j++)
					{
						...
						{
							int val =(int) (ip.getPixelValue(j,i));
			...
			for (int superi = ai.getAndIncrement(); superi <= dimZ; superi = ai.getAndIncrement()) {
				...
				for(int j=-1;j<dimY;j++)
				{
						for(int i=-1;i<dimX;i++)
						{
					        ...
					        else p[0]=(int)ip1.getPixelValue(j,i);

					        if (j==dimY-1 || i<0 || k<0) p[1]=(short)0;
					        else p[1]=(int)ip1.getPixelValue(j+1,i);

					        if(k==dimZ-1 || i<0 || j<0) p[2]=(short)0;
					        else p[2]=(int)ip2.getPixelValue(j,i);

					        if(k==dimZ-1 || j==dimY-1 || i<0) p[3]=(short)0;
					              else p[3]=(int)ip2.getPixelValue(j+1,i);

					        if(i==dimX-1 || k<0 || j<0) p[4]=(short)0;
					        else p[4]=(int)ip1.getPixelValue(j,i+1);

					        if(i==dimX-1 || j==dimY-1 || k<0) p[5]=(short)0;
					        else p[5]=(int)ip1.getPixelValue(j+1,i+1);

					        if(k==dimZ-1 || i==dimX-1 || j<0) p[6]=(short)0;
					        else p[6]=(int)ip2.getPixelValue(j,i+1);

					        if(k==dimZ-1 || j==dimY-1 || i==dimX-1) p[7]=(short)0;
					        else p[7]=(int)ip2.getPixelValue(j+1,i+1);
			...
			for (int superi = ai.getAndIncrement(); superi < dimZ-1; superi = ai.getAndIncrement()) {
				...
				for(int j=0;j<dimY-1;j++)
				{
						for(int i=0;i<dimX-1;i++)
						{
							...
							ptot=(short)0;
					        p[0]=(int)ip1.getPixelValue(j,i);
					        p[1]=(int)ip1.getPixelValue(j+1,i);
					        p[2]=(int)ip2.getPixelValue(j,i);
					        p[3]=(int)ip2.getPixelValue(j+1,i);
					        p[4]=(int)ip1.getPixelValue(j,i+1);
					        p[5]=(int)ip1.getPixelValue(j+1,i+1);
					        p[6]=(int)ip2.getPixelValue(j,i+1);
					        p[7]=(int)ip2.getPixelValue(j+1,i+1);
				...
				for(int j=0;j<dimY;j++)
					{
						for(int i=0;i<dimX;i++)
						{
							int pix =(int) (ip.getPixelValue(j,i));
							if(pix!=0){
						          tmpxg[k][pix-2] += j;
						          tmpyg[k][pix-2] += i;
						          tmpzg[k][pix-2] += k;
